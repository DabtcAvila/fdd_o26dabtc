...
No son ejemplos inventados. Son tres cosas que rompieron en las últimas cuatro unidades.

**A. El script de bash de [[bash-scripting|la unidad 5]].** Corre en tu máquina, se lo pasas a tu compañero y ahí devuelve otra cosa — o nada. Su `bash` es el 3.2 que Apple congeló en 2007, o su `date` es el de BSD y no entiende `-d`. Tu script está bien escrito. El intérprete es otro.

**B. El patrón de [[taquigrafia-perl|la unidad 6]].** `grep -P` no existe en macOS, y `grep -E '\d'` no busca dígitos: busca la letra `d`, sin avisar. El mismo patrón, sobre el mismo archivo, da dos resultados distintos según con qué `grep` lo corras. No hay error que leer.

**C. El pipeline de [[cuando-se-rompe|la unidad 2]].** Anda perfecto en tu laptop y muere en el servidor. A veces porque allá hay Python 3.8 y una versión vieja de `pandas`. A veces porque el CSV que leías vivía en `~/datos/` y allá ese directorio no existe.
...
El transporte marítimo vivió ese problema durante un siglo. La carga iba suelta —sacos, barriles, cajas—, y en cada trasbordo un equipo de estibadores la bajaba pieza por pieza y la volvía a acomodar. Estibar carga suelta costaba alrededor de `$5.86` por tonelada; el mismo trabajo con contenedores, `$0.16` (Levinson, *The Box*, 2006, con cifras de 1956).
...
Hasta aquí el contenedor parece una herramienta de reproducibilidad, y el mundo no se movió a algo tan grande sólo por eso. La otra mitad es que un artefacto sellado e idéntico a sí mismo no sólo corre igual **en otra máquina**: corre igual **mil veces a la vez**, en mil máquinas, sin que nadie instale nada. Quién decide esas mil copias tiene nombre —orquestación— y es la página 5 de esta sección.
...
**C2: no.** Un contenedor empaqueta software, no datos. Tu CSV no está adentro, y meterlo adentro sería peor, no mejor: la imagen se vuelve pesada e intransferible, y el contenedor es desechable. **Dónde vive ese archivo** es la pregunta que abre la sección 2, y **por qué no puede vivir adentro** es la de la página 5.

El corte es el útil: el contenedor te quita las diferencias de entorno de encima y te deja, limpia, la única decisión que sí era tuya.
:::
